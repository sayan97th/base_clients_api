...
class AdminNewContentTierController extends Controller
{
    ...
    {
        $plans = NewContentTier::withCount('orders as orders_count')
            ->withSum('orders as revenue_total', 'total_amount')
            ->orderBy('sort_order')
            ->get();

        return response()->json($plans->map(fn (NewContentTier $plan) => $this->formatPlan($plan)));
    }

    public function show(string $id): JsonResponse
    {
        $plan = NewContentTier::withCount('orders as orders_count')
            ->withSum('orders as revenue_total', 'total_amount')
            ->find($id);
    ...
    public function store(StoreNewContentTierRequest $request): JsonResponse
    {
        ...
        $plan = DB::transaction(function () use ($data): NewContentTier {
    ...
    private function formatPlan(NewContentTier $plan): array
    {
        return [
            'id'                   => $plan->id,
            'name'                 => $plan->name,
