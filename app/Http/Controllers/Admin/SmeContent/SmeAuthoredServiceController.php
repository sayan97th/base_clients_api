...
class SmeAuthoredServiceController extends Controller
{
    public function index(): JsonResponse
    {
        $services = SmeAuthoredTier::orderBy('sort_order')->get();
    ...
    public function store(StoreSmeServiceRequest $request): JsonResponse
    {
        ...
        $data['tier_key'] = $this->uniqueTierKey(SmeAuthoredTier::class, $data['label']);

        $service = SmeAuthoredTier::create($data);
    ...
    {
        $service = SmeAuthoredTier::where('tier_key', $service_id)->first();
    ...
    private function formatService(SmeAuthoredTier $service): array
    {
        return [
            'id'          => $service->tier_key,
            'label'       => $service->label,
