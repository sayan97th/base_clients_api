...
class SmeEnhancedServiceController extends Controller
{
    public function index(): JsonResponse
    {
        $services = SmeEnhancedTier::orderBy('sort_order')->get();
    ...
    public function store(StoreSmeServiceRequest $request): JsonResponse
    {
        ...
        $data['tier_key'] = $this->uniqueTierKey(SmeEnhancedTier::class, $data['label']);

        $service = SmeEnhancedTier::create($data);
    ...
    {
        $service = SmeEnhancedTier::where('tier_key', $service_id)->first();
    ...
    private function formatService(SmeEnhancedTier $service): array
    {
        return [
            'id'          => $service->tier_key,
            'label'       => $service->label,
