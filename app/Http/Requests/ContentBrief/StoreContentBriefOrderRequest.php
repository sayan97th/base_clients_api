...
namespace App\Http\Requests\ContentBrief;

use App\Models\ContentBriefTier;
use Illuminate\Foundation\Http\FormRequest;

class StoreContentBriefOrderRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'items.*.tier_id'                    => ['required', 'string', 'exists:content_brief_tiers,id'],
            'items.*.quantity'                   => ['required', 'integer', 'min:1'],
            'items.*.unit_price'                 => ['required', 'numeric', 'min:0'],
            'items.*.intake_rows'                => ['nullable', 'array'],
            'items.*.intake_rows.*.primary_keyword'    => ['required', 'string', 'max:255'],
            'items.*.intake_rows.*.secondary_keywords' => ['nullable', 'string', 'max:500'],
            'items.*.intake_rows.*.content_page_url'   => ['required', 'url', 'max:2048'],
    ...
    public function withValidator(\Illuminate\Contracts\Validation\Validator $validator): void
    {
        $validator->after(function ($validator) {
            foreach ($this->input('items', []) as $index => $item) {
                ...
                $tier = ContentBriefTier::where('id', $tier_id)
