...
return new class extends Migration
{
    ...
    {
        if (DB::getDriverName() !== 'sqlite') {
            DB::statement("ALTER TABLE link_building_orders MODIFY COLUMN status ENUM('pending','processing','completed','cancelled') NOT NULL DEFAULT 'pending'");
            DB::statement("ALTER TABLE new_content_orders MODIFY COLUMN status ENUM('pending','processing','completed','cancelled') NOT NULL DEFAULT 'pending'");
            DB::statement("ALTER TABLE content_optimization_orders MODIFY COLUMN status ENUM('pending','processing','completed','cancelled') NOT NULL DEFAULT 'pending'");
            DB::statement("ALTER TABLE content_brief_orders MODIFY COLUMN status ENUM('pending','processing','completed','cancelled') NOT NULL DEFAULT 'pending'");
        }
    }
};
