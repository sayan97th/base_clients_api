...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::getDriverName() !== 'sqlite') {
            DB::statement("ALTER TABLE link_building_orders MODIFY COLUMN status ENUM('new_request','pending','processing','completed','cancelled','payment_pending','pending_details') NOT NULL DEFAULT 'pending'");
            DB::statement("ALTER TABLE new_content_orders MODIFY COLUMN status ENUM('new_request','pending','processing','completed','cancelled','payment_pending','pending_details') NOT NULL DEFAULT 'pending'");
            DB::statement("ALTER TABLE content_optimization_orders MODIFY COLUMN status ENUM('new_request','pending','processing','completed','cancelled','payment_pending','pending_details') NOT NULL DEFAULT 'pending'");
            DB::statement("ALTER TABLE content_brief_orders MODIFY COLUMN status ENUM('new_request','pending','processing','completed','cancelled','payment_pending','pending_details') NOT NULL DEFAULT 'pending'");
        }
    }
